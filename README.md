...
Das Repository bleibt aus Gründen der Historie und bestehenden Integrationen unter `level13`. Die bestehende Browser-Spielarchitektur mit RequireJS, jQuery und Ash.js bleibt erhalten. Ausgeliefert wird sie über eine Node.js-22-Anwendung mit Express und Socket.IO.
...
Der Statusendpunkt liegt unter `/healthz` und meldet den Dienst als `sublevel`.

## Sprachen

Deutsch ist die Standardsprache. Alte Spielstände, die Englisch nur als früheren technischen Standard gespeichert haben, werden einmalig auf `DE_DE` migriert. Eine danach bewusst ausgewählte Sprache bleibt erhalten.
...
Fehlt eine optionale Sprachdatei vorübergehend, startet Sublevel weiterhin mit den vollständigen englischen Rückfalltexten, statt den gesamten Spielstart abzubrechen. Zusätzlich übersetzt `src/text/LocaleBootstrap.js` ältere, direkt im HTML hinterlegte Oberflächentexte und stellt sie beim Wechsel zu Englisch oder Finnisch wieder her.
...
- `src/text/TextLoader.js` – Sprachwahl, Migration und Übersetzungs-Fallback
- `src/text/LocaleBootstrap.js` – Sublevel-Branding und Übersetzung älterer statischer Texte
